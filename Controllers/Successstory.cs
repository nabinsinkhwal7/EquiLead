...
{
    public class Successstory : Controller
    {
        private readonly EquiDbContext _context;
        private readonly string _connectionString;

        public Successstory(EquiDbContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        {
            var existingRecords = _context.Tblsuccesstests.Where(x => x.IsDeleted == null || x.IsDeleted == false).ToList();
            var model = new SuccessstoryViewModel
            {
                Records = existingRecords,
                NewEntry = new Tblsuccesstest() // Initialize the model for the new entry form
            };
        ...
        [HttpPost]
        public async Task<IActionResult> Create(SuccessstoryViewModel model, IFormFile FileToUpload1)
        {
             // Handle the first file upload (Image)
                ...
                var imagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "assets", "SuccessPhoto", imageFileName);
                ...
                var directoryPath = Path.GetDirectoryName(imagePath);
                    ...
                    try
                    {
                        ...
                        model.NewEntry.Photo = "/assets/SuccessPhoto/" + imageFileName; // Relative path for front-end usage
                }
                    catch (Exception ex)
                    {
                ...
                // Save the new entry
                _context.Tblsuccesstests.Add(model.NewEntry);
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(SuccessstoryViewModel model, IFormFile FileToUpload1)
        {
            
                var record = _context.Tblsuccesstests.Find(model.NewEntry.Sstid);
                if (record != null)
                {
                    ...
                    if (FileToUpload1 != null && FileToUpload1.Length > 0)
                    {
                        ...
                        record.Photo = "/assets/SuccessPhoto/" + imageFileName; // Relative path for front-end usage
                    }
        ...
        {
            var record = _context.Tblsuccesstests.Find(id);
            if (record != null)
            {
